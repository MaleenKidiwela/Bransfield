function [subArrival] = subset_tlArrival(tlArrival, aPhase, aStation, aEvent, aEventType)
%SUBSET_tlARRIVAL -- Choose a subset of tlArrival
%                    (TomoLab Utility)
%
%   [subArrival] = subset_tlArrival(tlArrival, aPhase, aStation, aEvent, aEventType)
...
if nargin ~= 5
    error('5 input arguments: subset_tlArrival(tlArrival, aPhase, aStation, aEvent, aEventType)')
end

if isempty(aPhase) && isempty(aStation) && isempty(aEvent) && isempty(aEventType)
    error('subset_tlArrival:  One of the last 4 input arguments must be non-empty')
end
...
if ~isempty(aStation)
    ...
    if any(jsta)
        ...
        disp(['Skipping station(s) (', cell2string(aStation), ') No arrivals (subset_tlArrival)'])
        return
    end
end
...
%         disp(['Skipping event(s) (', int2str(aEvent),') No arrivals (subset_tlArrival)'])
        disp('Skipping event(s). No arrivals (subset_tlArrival)')
        return
    end
end
...
if ~isempty(aPhase)
    ...
    if any(jphs)
        ...
        disp(['Skipping phase(s) (', cell2string(aPhase),') No arrivals (subset_tlArrival)'])
        return
    end
end
...
if ~isempty(aEventType)
    ...
    if any(jtyp)
        ...
        disp(['Skipping eventtype(s) (', int2str(aEventType),') No arrivals (subset_tlArrival)'])
        return
    end
end
