...
%% Run through all shots per given OBS
i = obs_num;   %Manually typing in OBS's numbers.   %%%%length(obs.name);% For each OBS
...
waterPick(i).sdv = sdv;
cd '/Volumes/research/users/awells3/OBS_&_Event_Locations/Water_Wave_Picks'
eval(['save waterPick_OBS_' int2str(i) ' waterPick'])  % Saving the results up to now to an intermediate file.  eval(['save waterPick' date ' waterPick'])
