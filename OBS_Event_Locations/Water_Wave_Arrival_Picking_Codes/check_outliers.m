...
disp(['or (2) to Look at a specific Event.   ']);
eva = input(['']);
if eva == 1;
...
shotid = s.srEvent.id(find(~isnan(misfit(:,obs_num))));
    ...
    for k = 1:length(event);
        ...
        seisar = seis(istart+1:istart+twin/sampint,outlier_ind(k));
        seisar_unfilt = seis_unfilt(istart+1:istart+twin/sampint,outlier_ind(k));
        kst=1;
        ken=length(seisar);
        klo=kst+2*arord+1;
        khi=ken-2*arord-1;
        ...
        artime(k) =  s.time(find(event(k,1)==s.srEvent.id),obs_num);  %hdr(k).delay/1000 + (istart+index)*sampint;
...
%             end
            xlabel('time, sec');
            ylabel('Amplitude');
            title([' OBS# ' int2str(obs.name(obs_num_entry)) ', Channel ' int2str(chan) ', Shot Number ' int2str(shotid(outlier_ind(k))) ', Shot ' int2str(k) ' of ' int2str(length(event)) ],'FontSize',11)
            ...
            plot(t(istart2+1:istart2+twin2/sampint),seis(istart2+1:istart2+twin2/sampint,outlier_ind(k)))
            hold on
            yy = ylim;
            ...
            t = hdr(1).delay/1000 + (1:hdr(1).nsamp)*sampint;
            plot(t(istart2+1:istart2+twin2/sampint),seis_unfilt(istart2+1:istart2+twin2/sampint,outlier_ind(k)) - ...
                mean(seis_unfilt(istart2+1:istart2+twin2/sampint,outlier_ind(k))),'--')
            hold on
            yy = ylim;
